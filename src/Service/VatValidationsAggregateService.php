 ...
 */
final class VatValidationsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private VatValidationRepository $vatValidationRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'vat_validations' => $this->vatValidationRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\VatValidations\ModuleException("Record {$idStr} not found for locking.");
            }
