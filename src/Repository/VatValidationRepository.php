...
use BlackCat\Database\Packages\VatValidations\Dto\VatValidationDto as Dto;
use BlackCat\Database\Packages\VatValidations\Mapper\VatValidationDtoMapper as RowMapper;
...
final class VatValidationRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\VatValidations\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\VatValidations\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\VatValidations\Dto\VatValidationDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\VatValidations\Dto\VatValidationDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\VatValidations\Mapper\VatValidationDtoMapper::fromRow($row);
    }
