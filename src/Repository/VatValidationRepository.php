...
namespace BlackCat\Database\Packages\VatValidations\Repository;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\VatValidations\Definitions;
use BlackCat\Database\Packages\VatValidations\Criteria;
use BlackCat\Database\Packages\VatValidations\ContractRepository;

final class VatValidationRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO vat_validations (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO vat_validations (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO vat_validations (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE vat_validations SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE vat_validations SET $set WHERE id = :id", $params);
        }
        return $this->db->execute("DELETE FROM vat_validations WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE vat_validations SET $set WHERE id = :id", [':id'=>$id]);
    }

    // ============ READ / PAGE / LOCK ============
    public function findById(int|string $id): ?array {
        $sql = "SELECT * FROM vat_validations WHERE id = :id";
        $row = $this->db->fetchOne($sql, [':id'=>$id]);
        return $row ?: null;
    }

    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM vat_validations WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM vat_validations WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM vat_validations t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM vat_validations t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM vat_validations WHERE id = :id FOR UPDATE"
            : "SELECT * FROM vat_validations WHERE id = :id FOR UPDATE";
